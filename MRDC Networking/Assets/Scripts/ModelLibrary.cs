...
public class ModelLibrary : MonoBehaviour, IPunObservable
{
    #region Singleton
    /// <summary>
    /// Singleton instance
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        if (stream.IsWriting)
        {
            ...
            stream.SendNext(hololensModel.activeSelf);
            stream.SendNext(carModel.activeSelf);
            stream.SendNext(engineModel.activeSelf);
        }
        ...
        else if (stream.IsReading)
        {
            ...
            hololensModel.SetActive((bool)stream.ReceiveNext());
            carModel.SetActive((bool)stream.ReceiveNext());
            engineModel.SetActive((bool)stream.ReceiveNext());
        }
    }
}
